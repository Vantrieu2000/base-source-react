...
import { CustomHandleError, CustomHandleSuccess } from "../../utils/api/responseHandler";
...
export const AddOptions = (props: WrappedComponentProps) => {
  ...
  // create option
  const { mutate: CreateOption, status: statusCreateOption } = useMutation(api.Option.createOption, {
    onSuccess: (data) => {
      if (data.status === 200) {
  ...
  // Delete option
  const { mutate: DeleteOption, status: statusDeleteCategory } = useMutation(api.Option.deleteOption, {
    onSuccess: (data) => {
      if (data.status === 200) {
        CustomHandleSuccess(data.status, intl);
        reset();
  ...
  //Update option
  const { mutate: UpdateOption, status: statusUpdateOption } = useMutation(api.Option.editOption, {
    onSuccess: (data) => {
      CustomHandleSuccess(data.status, intl);
      reset();
  ...
  const modalConfirmDeleteOption = (item) => {
    ConfirmAction(
      ...
      {
        title: "Delete",
        content: item.isTemplate ? "This is template item !!!" : "Are you sure you want to delete?",
        okText: "Confirm",
        cancelText: "Cancel",
      }
    );
  ...
  return (
    <div className="option-setting" id="option-setting">
      <div className="option-setting__container">
        <div className="option-setting__container__title">{intl.formatMessage({ id: "pages.store-option.title" })}</div>
          ...
          <div className="col-xs-6 col-md-4 option-setting__container__content__list">
            <div className="option-setting__container__content__list__search">
              <Form form={searchForm}>
                <Form.Item name={"name"}>
                  <Search placeholder="Search here" allowClear enterButton size="large" onSearch={onSearch} />
                </Form.Item>
              </Form>
            </div>
            <div className="option-setting__container__content__list__options">
              ...
              <div className="option-setting__container__content__list__options__content">
                {data?.length > 0 ? (
                  data?.map((item, index) => (
                    <div
                      className={clsx(`option-setting__item`, selectedId === item._id && !item?.isTemplate && "active")}
                      key={index + item._id}
                      onClick={() => handleSelectItem(item._id)}
                    >
                      <div className="option-setting__item__name">{item.name}</div>
          ...
          <div className="col-xs-12 col-md-8 option-setting__container__content__detail">
            <div className="option-setting__detail">
                        ...
                        return (
                          <div className="option-setting__detail__form__modifier__list">
                            {fields.map((field, index) => (
                              <div className="option-setting__detail__form__modifier__list__item" key={field.key}>
                                <Form.Item
                                  name={[index, "status"]}
                                  style={{ width: "12%" }}
                                  initialValue={false}
              ...
              <div className="option-setting__detail__action">
                ...
                <CustomButton type="primary" onClick={form.submit} disabled={disableButton}>
