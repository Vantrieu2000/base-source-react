...
export const CategorySetting = (props: WrappedComponentProps) => {
  ...
  const { mutate: CreateCategory, status: statusCreateCategory } = useMutation(api.Category.createCategory, {
    onSuccess: (data) => {
  ...
  const { mutate: UpdateCategory, status: statusUpdateCategory } = useMutation(api.Category.editCategory, {
    onSuccess: (data) => {
      CustomHandleSuccess(data.status, intl);
      reset();
      queryClient.invalidateQueries(["category-list", store_id]);
  ...
  const { mutate: DeleteCategory, status: statusDeleteCategory } = useMutation(api.Category.deleteCategory, {
    onSuccess: (data) => {
      CustomHandleSuccess(data.status, intl);
      reset();
      queryClient.invalidateQueries(["category-list", store_id]);
  ...
  const onDragEnd = (result) => {
    ...
    const items = reorder(listCategory, result.source.index, result.destination.index);
                    ...
                    {listCategory?.map((item, index) => (
                      <Draggable key={item._id} draggableId={item._id} index={index}>
                        {(provided, snapshot) => (
                          ...
                          >
                            <div {...provided.dragHandleProps} className="category-setting__item__move">
                              <DragOutlined style={{ fontSize: "24px" }} />
                            </div>
                            <div className="category-setting__item__name">{item.name}</div>
            ...
            {selectedId || actionType === "create" ? (
              <div className="category-setting__detail">
                ...
                <div className="category-setting__detail__action">
                  ...
                  <CustomButton type="primary" disabled={disableButton} onClick={form.submit}>
                    {intl.formatMessage({ id: "system.save" })}
                  </CustomButton>
                </div>
              </div>
            ) : (
              <div className="category-setting__detail no-category">No Category Selected.</div>
            )}
          </div>
        </div>
      </div>
